(function($) {
	...
	window.MediaCenter.Gallery	= function(dialog) {
		...
		this.onImageSelected = this.dialog.options.onImageSelected;
	...
	window.MediaCenter.Gallery.prototype = {
		...
		getReferrerEl: function() {
			return this.getDialog().getReferrerEl();
		},
		...
		onMediaImageLinkClick: function(el, evt) {
			...
			if (this.ckeditor) {
				...
				var src = el.data('src');
				this.getReferrerEl().val(src);
				this.addImageToPreview(src);

				this.onImageSelected.call(src, this.getDialog());
				this.close();
			}
		},

		addImageToPreview: function(src) {
			var preview = this.getDialog().getPreviewEl();
