...
class Post extends \Speedy\Model\ActiveRecord {
	...
	public function meta() {
		return [
			'description' => isset($this->custom_data['meta_description']) ?
				$this->custom_data['meta_description'] : '',
			'keywords'	=> isset($this->custom_data['meta_keywords']) ?
				$this->custom_data['meta_keywords'] : '',
			'author'	=> isset($this->author) ?
				$this->author->name : ''
		];
	}

	public static function recent($types = [], $limit = 5) {
		if (empty($types))
			$types[] = self::PostType;
	...
	public function hasFeaturedImage() {
		return isset($this->featured_image) && strlen($this->featured_image) > 0;
	}
}
