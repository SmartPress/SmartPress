<?php $menus->each(function($menu) use ($level) { ?>
	<li class="ui-state-default level-<?php echo $level ?>" data-id="<?php echo $menu->id; ?>">
		...
		<?php echo $menu->title; ?>
		<?php $this->linkTo('<i class="icon-remove-sign icon-white"></i>', $this->admin_menu_path($menu), ['class' => 'btn btn-mini btn-danger pull-right', 'confirm' => 'Are you sure?', 'method' => 'delete']); ?>
	</li>
